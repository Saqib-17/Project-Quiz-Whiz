...
import { useParams } from 'react-router-dom'; // To get the dynamic subject parameter
    ...
    <div className="flex-grow p-6 md:p-12 mx-4 lg:mx-60">
      <section className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {questions.map((question, index) => (
          <div key={index} className="bg-[#d4e5dc] shadow-lg rounded-lg p-4 md:p-6 hover:bg-[#c8e0d3] hover:glow-effect transition duration-300 mx-4 md:mx-6 my-4 md:my-6">
            <h2 className="text-xl font-semibold text-primary-color mb-2">{question.title}</h2>
            <p className="text-gray-700 mb-4">{question.text}</p>
            <div className="mt-4 grid grid-cols-2 gap-2">
              {question.options.map((option, idx) => (
                <label key={idx} className="flex items-center space-x-2">
                  <input type="radio" name={`q${index}`} className={`accent-${option.isCorrect ? 'green' : 'red'}-500`} />
